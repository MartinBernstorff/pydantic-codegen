...
from pydantic_codegen.module_source import ModuleSource, annotation_names, free_names
...
class Bindings:
    ...
    def imports_for(self, source: Expression) -> tuple[Import, ...]:
        return tuple(Arr(_names(source)).map(self._import_of).to_list())

    @cached_property
    def _bound(self) -> dict[SymbolName, Import]:
        return {
