...
from pydantic_codegen.loader import ModelTarget, parsed_target
...
def set_bases(*targets: str) -> Transformer:
    parsed = Arr(targets).map(ModelTarget).map(parsed_target).to_list()
    statements = tuple(
        Arr(parsed).map(lambda target: target.imports()).flatten().to_list()
    )
    bases = tuple(Arr(parsed).map(lambda target: target.base()).to_list())
    return each(
